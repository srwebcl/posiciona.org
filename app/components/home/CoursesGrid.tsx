    ...
    return (
        <section className="py-24 bg-transparent relative">
            <div className="container mx-auto px-4">
                ...
                <div className="flex overflow-x-auto hide-scrollbar gap-3 mb-16 pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap md:justify-center snap-x">
                    {FILTERS.map((filter) => (
                        <button
                            key={filter.id}
                            onClick={() => setActiveFilter(filter.id)}
                            className={`
                relative px-6 py-2.5 rounded-full text-sm font-bold tracking-wide transition-all duration-300 z-10 whitespace-nowrap shrink-0 snap-center
                ...
                <motion.div layout className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <AnimatePresence mode="popLayout">
                        {filteredCourses.slice(0, 6).map((course) => (
                            ...
                            >
                                <div className="h-full bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 hover:border-blue-100 transition-all duration-300 group flex flex-col">
                                    {/* Card Header (Image) */}
                                    <div className="relative h-48 w-full overflow-hidden bg-gray-100">
                                        ...
                                        />
                                        <div className="absolute inset-0 bg-gradient-to-t from-navy-deep/80 to-transparent opacity-60 group-hover:opacity-80 transition-opacity"></div>

                                        {/* Icon overlay */}
                                        <div className={`absolute bottom-4 left-4 p-2 rounded-lg backdrop-blur-md ${course.color === 'amber' ? 'bg-amber-500/90 text-white' :
                                            course.color === 'cyan' ? 'bg-cyan-500/90 text-white' :
